...
export async function callAIWithFallback<T>(
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      const data = await callSarvamStructured<T>(
        systemPrompt, userPrompt, jsonStructureHint,
        temperature, sarvamModel, maxTokens
      );
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      const data = await callGroqStructured<T>(
        systemPrompt, userPrompt, jsonStructureHint, temperature
      );
      logUsage(functionName, undefined, maxTokens);
  ...
  throw new Error(
    `All AI providers failed. Last error: ${lastError?.message || "Unknown error"}. Please try again in a few minutes.`
  );
}

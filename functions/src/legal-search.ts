...
import { callGeminiStructured, callGeminiText } from "./gemini-client";
...
async function searchIndianKanoon(
  query: string,
  options?: { fromdate?: string; todate?: string; doctypes?: string; maxResults?: number; pagenum?: number }
...
async function getIndianKanoonDoc(tid: string): Promise<string> {
  if (!IK_TOKEN) return "";
...
// Google CSE replaced with Gemini — no API key needed beyond GEMINI_API_KEY
// const GOOGLE_CSE_API_KEY removed — using Gemini for web search
// const GOOGLE_CSE_ID removed — using Gemini for web search
 ...
 */
async function searchGoogleCSE(query: string, num = 8): Promise<WebSearchResult[]> {
...
- Be specific with citations when possible`;
    ...
    const response = await callGeminiText(
      "You are an expert Indian legal researcher with deep knowledge of case law. Always provide accurate, real case names and citations.",
      prompt,
      0.3
    );
    ...
    const caseBlocks = response.split(new RegExp("CASE:\\s*", "i")).filter((b: string) => b.trim());
    ...
    for (const block of caseBlocks.slice(0, num)) {
      ...
      for (const ln of lines2) {
        if (/^COURT:\s*/i.test(ln)) court = ln.replace(/^COURT:\s*/i, "").trim();
        else if (/^YEAR:\s*/i.test(ln)) year = ln.replace(/^YEAR:\s*/i, "").trim();
        else if (/^CITATION:\s*/i.test(ln)) citation = ln.replace(/^CITATION:\s*/i, "").trim();
        else if (/^RELEVANCE:\s*/i.test(ln)) relevance = ln.replace(/^RELEVANCE:\s*/i, "").trim();
      }
    ...
    if (results.length === 0) {
      ...
      try {
        const sarvamResult = await callSarvamStructured<any>(
          `You are an expert Indian legal researcher. Given: "${query}". Return real Indian legal cases.`,
          `List landmark Indian legal cases for: "${query}"`,
          `{"cases":[{"name":"Petitioner vs Respondent","court":"Supreme Court of India","year":"2020","citation":"(2020) 10 SCC 123","relevance":"Why relevant"}]}`,
          0.2, "sarvam-105b", 4000
        );
        ...
        console.error("[legal-search] Sarvam knowledge fallback also failed:", (e as Error)?.message);
      }
    }
...
async function fetchWebContentForResults(webResults: WebSearchResult[]): Promise<{ url: string; text: string }[]> {
  const priorityHosts = [
    "livelaw.in",
    "indiankanoon.org",
    "supremecourt.gov.in",
  ...
  await Promise.all(sorted.slice(0, 2).map(async (result) => { // Fetch 6 instead of 5
...
interface ParsedCaseQuery {
  raw: string;
  parties: { petitioner?: string; respondent?: string };
  partyNameWords: string[];
  caseNumbers: string[];
  isCaseReference: boolean;
  year?: string;
  topicKeywords: string[];
  /** Multiple IK queries for parallel search */
  ikQueries: string[];
  /** Clean query for Google CSE */
  cseQuery: string;
}

function parseCaseQuery(query: string): ParsedCaseQuery {
  const raw = query;
  const parties: { petitioner?: string; respondent?: string } = {};
  ...
  const vsMatch = query.match(
    /^(.+?)\s+(?:vs\.?|v\.?|versus|Vs\.?|V\.?)\s+(.+?)(?:\s*,\s*(?:MA|WP|CA|SLP|Crl|FA|SA|IA|TA|Misc|Writ|Arb)|\s*$)/i
  );
  if (vsMatch) {
  ...
  for (const pattern of caseNumPatterns) {
    ...
    if (matches) caseNumbers.push(...matches.map(m => m.trim()));
  }
  ...
  const partyNameWords: string[] = [];
  const addPartyWords = (name: string) => {
  ...
  if (parties.petitioner) addPartyWords(parties.petitioner);
  if (parties.respondent) addPartyWords(parties.respondent);
  ...
  if (uniqueQueries.length === 0) uniqueQueries.push(raw); // Fallback to raw
...
function mergeDedupResults(allResults: IKSearchResult[][]): IKSearchResult[] {
  const seenTids = new Set<string>();
  const merged: IKSearchResult[] = [];
  const tidSources = new Map<string, number>(); // Count how many queries found each result
...
function validateAndRankResults(
  ikResults: IKSearchResult[],
  parsedQuery: ParsedCaseQuery
): IKSearchResult[] {
  ...
  if (matching.length > 0) {
    ...
    for (const nm of nonMatching.slice(0, 3)) {
      if (kept.length < 8) kept.push(nm.result);
    }
...
export interface ResearchData {
  ...
  fetchedDocs: { tid: string; text: string }[];
  hasRealData: boolean;
  totalFound: string;
  dataSources: string[];
  parsedQuery: ParsedCaseQuery;
...
function isRecentCaseQuery(query: string, year?: string): boolean {
  if (year) {
    const y = parseInt(year);
    if (!isNaN(y) && y >= 2025) return true; // Lowered to 2025 — IK may lag
  }
...
export async function performLegalSearch(
  ...
  const ikResults: IKSearchResult[] = [];
  const webResults: WebSearchResult[] = [];
  const fetchedDocs: { tid: string; text: string }[] = [];
  ...
  let doctypes = "";
  if (options?.court === "supreme") doctypes = "supremecourt";
  else if (options?.court === "high") doctypes = "judgments";
  else if (options?.court) doctypes = options.court.toLowerCase();
  ...
  if (fromdate || todate) {
    ...
    for (let i = 0; i < datedResults.length; i++) {
      if (datedResults[i].status === "fulfilled" && (datedResults[i] as PromiseFulfilledResult<IKSearchResult[]>).value.length > 0) {
        console.log(`[legal-search]   Dated query → ${(datedResults[i] as PromiseFulfilledResult<IKSearchResult[]>).value.length} results`);
      }
    }
    // Merge dated results into main results
    for (const r of datedResults) {
      if (r.status === "fulfilled") ikAllResults.push(r);
    }
  }

  // MERGE and DEDUP all IK results
  const allIKArrays = ikAllResults
    .filter((r): r is PromiseFulfilledResult<IKSearchResult[]> => r.status === "fulfilled")
    .map(r => r.value);
  ...
  if (isRecent || parsed.isCaseReference || ikResults.length < 5) {
    ...
    try {
      ...
      if (webSearchResults.length > 0) dataSources.push("Web Search");

      // Only fetch web content for results with real URLs (skip Gemini knowledge results)
      const realWebResults = webSearchResults.filter((r: any) => r.url && r.url.startsWith("http"));
 ...
 */
export async function analyzeLegalData(data: ResearchData): Promise<{
  results: LegalCase[];
  ...
  let relatedPrecedents: { title: string; year: string; citation: string; source: string }[] = [];
...
async function extractSingleCase(
  ikResult: IKSearchResult,
  docText: string,
  webContext: string,
  originalQuery: string,
  parsed: ParsedCaseQuery,
  index: number
  ...
  if (ikResult.headline) contextParts.push(`Headline: ${ikResult.headline}`);
...
Extract structured data from this case. Respond with valid JSON only.`;
  ...
  if (!result) {
    try {
      ...
      result = await callGeminiStructured<any>(systemPrompt, userPrompt, geminiSchema, 0.1);
  ...
  if (!result) {
    try {
      result = await callGroqStructured<any>(systemPrompt, userPrompt, schemaHint, 0.1);
  ...
  if (docText.length > 200) {
    const excerpts: LegalCase["sourceExcerpts"] = {};
    ...
    if (factsExcerpt) excerpts.facts = factsExcerpt;
    ...
    if (issuesExcerpt) excerpts.issues = issuesExcerpt;
    ...
    if (heldExcerpt) excerpts.held = heldExcerpt;
    ...
    if (ratioExcerpt) excerpts.ratio = ratioExcerpt;
    ...
    if (dispositionExcerpt) excerpts.disposition = dispositionExcerpt;
    ...
    if (docText.length > 500) {
      ...
      const rulingIdx = Math.max(
        lastHalf.toLowerCase().lastIndexOf("accordingly"),
        lastHalf.toLowerCase().lastIndexOf("allowed"),
        lastHalf.toLowerCase().lastIndexOf("dismissed"),
        lastHalf.toLowerCase().lastIndexOf("disposed"),
        lastHalf.toLowerCase().lastIndexOf("petition is"),
        lastHalf.toLowerCase().lastIndexOf("appeal is"),
        lastHalf.toLowerCase().lastIndexOf("in the result"),
      );
      if (rulingIdx > 0) {
...
function extractSection(text: string, patterns: string[], excerptLength = 800): string | undefined {
...
async function generateSuggestedArguments(
  cases: LegalCase[],
  originalQuery: string,
  parsed: ParsedCaseQuery
): Promise<{
  arguments: string[];
  precedents: { title: string; year: string; citation: string; source: string }[];
}> {
  // Build a concise context of all extracted cases for argument generation
  const caseSummaries = cases.slice(0, 4).map(c =>
    `[${c.title}] (${c.court}, ${c.year}): ${c.summary.substring(0, 300)}... Holdings: ${(c.keyHoldings || []).slice(0, 2).join("; ")}`
  ).join("\n\n");
...
Generate suggested arguments and related precedents. Respond with valid JSON.`;
  ...
  try {
    const result = await callSarvamStructured<any>(systemPrompt, userPrompt, schemaHint, 0.3, "sarvam-105b", 4000);
    return {
      arguments: (result.arguments || []).filter((a: string) => a.length > 20),
      precedents: (result.precedents || []).map((p: any) => ({
        title: p.title || "",
        year: p.year || "",
        citation: p.citation || "",
        source: p.source || "",
    ...
    console.error("[arguments] Sarvam failed:", (e as Error)?.message);
  }

  try {
    const result = await callGroqStructured<any>(systemPrompt, userPrompt, schemaHint, 0.3);
    return {
      arguments: (result.arguments || []).filter((a: string) => a.length > 20),
      precedents: (result.precedents || []).map((p: any) => ({
        title: p.title || "",
    ...
    console.error("[arguments] Groq also failed:", (e as Error)?.message);
  }

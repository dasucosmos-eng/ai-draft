...
function buildCRPPrompt(
  crpType: string,
  impugnedOrder: any,
  petitionerDetails: any,
...
## Respondent Details
${respondentDetails.map((r: any, i: number) => `${i + 1}. **Respondent ${i + 1}** — ${r.designation || "Not specified"}, ${r.department || "Not specified"}, ${r.address || "Address not specified"}`).join("\n") || "No respondents specified"}
...
function buildParseFIRPrompt(firText: string): string {
...
function buildSuggestBailGroundsPrompt(
  firDetails: any,
  accusedDetails: any,
...
function buildCriminalAppealPrompt(
...
function normalizeWritType(val: string): string {
  if (!val) return "mandamus";
  const lower = val.toLowerCase().replace(/[\s\-_]/g, "");
  if (lower.includes("habeascorpus") || lower.includes("habeas")) return "habeas_corpus";
  if (lower.includes("certiorari")) return "certiorari";
  if (lower.includes("prohibition")) return "prohibition";
  if (lower.includes("quowarranto") || lower.includes("quowarranto")) return "quo_warranto";
...
function detectJurisdictionFromCourt(courtName: string): { jurisdiction: string; highCourtName?: string } {
  if (!courtName) return { jurisdiction: "high_court" };
  const lower = courtName.toLowerCase();
  if (lower.includes("supreme")) return { jurisdiction: "supreme_court" };
  // Try to extract high court name
  const hcMatch = lower.match(/high\s+court\s+of\s+(.+)/);
  if (hcMatch) return { jurisdiction: "high_court", highCourtName: courtName };
  // If it's just "High Court" without name, default
  if (lower.includes("high court")) return { jurisdiction: "high_court", highCourtName: courtName };
  return { jurisdiction: "high_court", highCourtName: courtName };
}

// ─── Helper: Detect crlmpType from petitionType ──────────────────────────────────

function detectCrlmpType(val: string): string {
  if (!val) return "interim_relief";
  const lower = val.toLowerCase().replace(/[\s\-_]/g, "");
  if (lower.includes("suspension") || lower.includes("sentence")) return "suspension_sentence";
  if (lower.includes("modification")) return "modification";
  if (lower.includes("direction")) return "directions";
  if (lower.includes("transfer")) return "transfer";
  if (lower.includes("expunction") || lower.includes("remarks")) return "expunction";
  if (lower.includes("quash")) return "quashing";
  if (lower.includes("leave") || lower.includes("appeal")) return "leave_to_appeal";
  if (lower.includes("interim")) return "interim_relief";
  return val;
}

// ─── Main Cloud Function ───────────────────────────────────────────────────────

export const apiAiCriminal = https.onRequest(
  {
    timeoutSeconds: 180,
      ...
      try {
        ...
        switch (task) {
          ...
          case "generateDocument": {
            ...
            switch (innerTask) {
              case "generateBail": {
                ...
                try {
                  data = await callSarvamStructured<typeof data>(SYSTEM_PROMPT, userPrompt, BAIL_JSON_STRUCTURE, 0.3, "sarvam-105b");
                } catch (sarvamErr) {
                  console.error("[ai-criminal] Sarvam failed for generateBail, falling back to Groq:", sarvamErr?.message);
                  try {
                    data = await callGroqStructured<typeof data>(SYSTEM_PROMPT, userPrompt, BAIL_JSON_STRUCTURE, 0.3);
                  } catch (groqErr) {
              ...
              case "generateCRP": {
                ...
                let data: { title: string; content: string; keyPoints: string[]; warnings: string[] };
                try {
                  data = await callSarvamStructured<typeof data>(SYSTEM_PROMPT, userPrompt, CRP_JSON_STRUCTURE, 0.3, "sarvam-105b");
                } catch (sarvamErr) {
                  console.error("[ai-criminal] Sarvam failed for generateCRP, falling back to Groq:", sarvamErr?.message);
                  try {
                    data = await callGroqStructured<typeof data>(SYSTEM_PROMPT, userPrompt, CRP_JSON_STRUCTURE, 0.3);
                  } catch (groqErr) {
              ...
              case "generateWrit": {
                ...
                let data: { title: string; content: string; keyPoints: string[]; warnings: string[] };
                try {
                  data = await callSarvamStructured<typeof data>(SYSTEM_PROMPT, userPrompt, WRIT_JSON_STRUCTURE, 0.3, "sarvam-105b");
                } catch (sarvamErr) {
                  console.error("[ai-criminal] Sarvam failed for generateWrit, falling back to Groq:", sarvamErr?.message);
                  try {
                    data = await callGroqStructured<typeof data>(SYSTEM_PROMPT, userPrompt, WRIT_JSON_STRUCTURE, 0.3);
                  } catch (groqErr) {
              ...
              case "generateCRLMP": {
                ...
                let data: { title: string; content: string; keyPoints: string[]; warnings: string[] };
                try {
                  data = await callSarvamStructured<typeof data>(SYSTEM_PROMPT, userPrompt, CRLMP_JSON_STRUCTURE, 0.3, "sarvam-105b");
                } catch (sarvamErr) {
                  console.error("[ai-criminal] Sarvam failed for generateCRLMP, falling back to Groq:", sarvamErr?.message);
                  try {
                    data = await callGroqStructured<typeof data>(SYSTEM_PROMPT, userPrompt, CRLMP_JSON_STRUCTURE, 0.3);
                  } catch (groqErr) {
              ...
              case "generateCriminalAppeal": {
                ...
                let data: { title: string; content: string; keyPoints: string[]; warnings: string[] };
                try {
                  data = await callSarvamStructured<typeof data>(SYSTEM_PROMPT, userPrompt, CRIMINAL_APPEAL_JSON_STRUCTURE, 0.3, "sarvam-105b");
                } catch (sarvamErr) {
                  console.error("[ai-criminal] Sarvam failed for generateCriminalAppeal, falling back to Groq:", sarvamErr?.message);
                  try {
                    data = await callGroqStructured<typeof data>(SYSTEM_PROMPT, userPrompt, CRIMINAL_APPEAL_JSON_STRUCTURE, 0.3);
                  } catch (groqErr) {
              ...
              case "parseFIR": {
                ...
                let data: { firNumber: string; policeStation: string; dateOfFir: string; sectionsCharged: string[]; underOldOrNewLaw: string; complainantName: string; accusedNames: string[]; briefFacts: string; offenceCategory: string; bailableStatus: string };
                try {
                  data = await callSarvamStructured<typeof data>(SYSTEM_PROMPT, userPrompt, PARSE_FIR_JSON_STRUCTURE, 0.3, "sarvam-105b");
                } catch (sarvamErr) {
                  console.error("[ai-criminal] Sarvam failed for parseFIR, falling back to Groq:", sarvamErr?.message);
                  try {
                    data = await callGroqStructured<typeof data>(SYSTEM_PROMPT, userPrompt, PARSE_FIR_JSON_STRUCTURE, 0.3);
                  } catch (groqErr) {
              ...
              case "suggestBailGrounds": {
                ...
                let data: { grounds: string[]; applicableBailType: string; relevantSections: string[]; strongestArguments: string[] };
                try {
                  data = await callSarvamStructured<typeof data>(SYSTEM_PROMPT, userPrompt, SUGGEST_BAIL_GROUNDS_JSON_STRUCTURE, 0.3, "sarvam-105b");
                } catch (sarvamErr) {
                  console.error("[ai-criminal] Sarvam failed for suggestBailGrounds, falling back to Groq:", sarvamErr?.message);
                  try {
                    data = await callGroqStructured<typeof data>(SYSTEM_PROMPT, userPrompt, SUGGEST_BAIL_GROUNDS_JSON_STRUCTURE, 0.3);
                  } catch (groqErr) {
          ...
          case "generateBail": {
            ...
            try {
              data = await callSarvamStructured<{
                ...
                keyPoints: string[];
                warnings: string[];
              }>(SYSTEM_PROMPT, userPrompt, BAIL_JSON_STRUCTURE, 0.3, "sarvam-105b");
            } catch (sarvamErr) {
              console.error("[ai-criminal] Sarvam failed for generateBail, falling back to Groq:", sarvamErr?.message);
              try {
                data = await callGroqStructured<{
                  ...
                  keyPoints: string[];
                  warnings: string[];
                }>(SYSTEM_PROMPT, userPrompt, BAIL_JSON_STRUCTURE, 0.3);
              } catch (groqErr) {
                console.error("[ai-criminal] Groq failed for generateBail, falling back to Gemini:", groqErr?.message);
                const geminiPrompt = `${SYSTEM_PROMPT}\n\nCRITICAL: Respond ONLY with valid JSON:\n${BAIL_JSON_STRUCTURE}`;
                const geminiResponse = await callGeminiText(geminiPrompt, userPrompt, 0.3);
          ...
          case "generateCRP": {
            ...
            try {
              data = await callSarvamStructured<{
                ...
                keyPoints: string[];
                warnings: string[];
              }>(SYSTEM_PROMPT, userPrompt, CRP_JSON_STRUCTURE, 0.3, "sarvam-105b");
            } catch (sarvamErr) {
              console.error("[ai-criminal] Sarvam failed for generateCRP, falling back to Groq:", sarvamErr?.message);
              try {
                data = await callGroqStructured<{
                  ...
                  keyPoints: string[];
                  warnings: string[];
                }>(SYSTEM_PROMPT, userPrompt, CRP_JSON_STRUCTURE, 0.3);
              } catch (groqErr) {
                console.error("[ai-criminal] Groq failed for generateCRP, falling back to Gemini:", groqErr?.message);
                const geminiPrompt = `${SYSTEM_PROMPT}\n\nCRITICAL: Respond ONLY with valid JSON:\n${CRP_JSON_STRUCTURE}`;
                const geminiResponse = await callGeminiText(geminiPrompt, userPrompt, 0.3);
          ...
          case "generateWrit": {
            ...
            try {
              data = await callSarvamStructured<{
                ...
                keyPoints: string[];
                warnings: string[];
              }>(SYSTEM_PROMPT, userPrompt, WRIT_JSON_STRUCTURE, 0.3, "sarvam-105b");
            } catch (sarvamErr) {
              console.error("[ai-criminal] Sarvam failed for generateWrit, falling back to Groq:", sarvamErr?.message);
              try {
                data = await callGroqStructured<{
                  ...
                  keyPoints: string[];
                  warnings: string[];
                }>(SYSTEM_PROMPT, userPrompt, WRIT_JSON_STRUCTURE, 0.3);
              } catch (groqErr) {
                console.error("[ai-criminal] Groq failed for generateWrit, falling back to Gemini:", groqErr?.message);
                const geminiPrompt = `${SYSTEM_PROMPT}\n\nCRITICAL: Respond ONLY with valid JSON:\n${WRIT_JSON_STRUCTURE}`;
                const geminiResponse = await callGeminiText(geminiPrompt, userPrompt, 0.3);
          ...
          case "parseFIR": {
            ...
            try {
              data = await callSarvamStructured<{
                ...
                offenceCategory: string;
                bailableStatus: string;
              }>(SYSTEM_PROMPT, userPrompt, PARSE_FIR_JSON_STRUCTURE, 0.3, "sarvam-105b");
            } catch (sarvamErr) {
              console.error("[ai-criminal] Sarvam failed for parseFIR, falling back to Groq:", sarvamErr?.message);
              try {
                data = await callGroqStructured<{
                  ...
                  offenceCategory: string;
                  bailableStatus: string;
                }>(SYSTEM_PROMPT, userPrompt, PARSE_FIR_JSON_STRUCTURE, 0.3);
              } catch (groqErr) {
                console.error("[ai-criminal] Groq failed for parseFIR, falling back to Gemini:", groqErr?.message);
                const geminiPrompt = `${SYSTEM_PROMPT}\n\nCRITICAL: Respond ONLY with valid JSON:\n${PARSE_FIR_JSON_STRUCTURE}`;
                const geminiResponse = await callGeminiText(geminiPrompt, userPrompt, 0.3);
          ...
          case "suggestBailGrounds": {
            ...
            try {
              data = await callSarvamStructured<{
                ...
                relevantSections: string[];
                strongestArguments: string[];
              }>(SYSTEM_PROMPT, userPrompt, SUGGEST_BAIL_GROUNDS_JSON_STRUCTURE, 0.3, "sarvam-105b");
            } catch (sarvamErr) {
              console.error("[ai-criminal] Sarvam failed for suggestBailGrounds, falling back to Groq:", sarvamErr?.message);
              try {
                data = await callGroqStructured<{
                  ...
                  relevantSections: string[];
                  strongestArguments: string[];
                }>(SYSTEM_PROMPT, userPrompt, SUGGEST_BAIL_GROUNDS_JSON_STRUCTURE, 0.3);
              } catch (groqErr) {
                console.error("[ai-criminal] Groq failed for suggestBailGrounds, falling back to Gemini:", groqErr?.message);
                const geminiPrompt = `${SYSTEM_PROMPT}\n\nCRITICAL: Respond ONLY with valid JSON:\n${SUGGEST_BAIL_GROUNDS_JSON_STRUCTURE}`;
                const geminiResponse = await callGeminiText(geminiPrompt, userPrompt, 0.3);
          ...
          case "generateCRLMP": {
            ...
            try {
              data = await callSarvamStructured<{
                ...
                keyPoints: string[];
                warnings: string[];
              }>(SYSTEM_PROMPT, userPrompt, CRLMP_JSON_STRUCTURE, 0.3, "sarvam-105b");
            } catch (sarvamErr) {
              console.error("[ai-criminal] Sarvam failed for generateCRLMP, falling back to Groq:", sarvamErr?.message);
              try {
                data = await callGroqStructured<{
                  ...
                  keyPoints: string[];
                  warnings: string[];
                }>(SYSTEM_PROMPT, userPrompt, CRLMP_JSON_STRUCTURE, 0.3);
              } catch (groqErr) {
                console.error("[ai-criminal] Groq failed for generateCRLMP, falling back to Gemini:", groqErr?.message);
                const geminiPrompt = `${SYSTEM_PROMPT}\n\nCRITICAL: Respond ONLY with valid JSON:\n${CRLMP_JSON_STRUCTURE}`;
                const geminiResponse = await callGeminiText(geminiPrompt, userPrompt, 0.3);
          ...
          case "generateCriminalAppeal": {
            ...
            const userPrompt = buildCriminalAppealPrompt(
              appellantName, respondentName, lowerCourt, appellateCourt,
              firNumber, policeStation, convictionDate, sectionsCharged,
              sentence, impugnedOrder, groundsOfAppeal, facts
            );
            ...
            try {
              data = await callSarvamStructured<{
                ...
                keyPoints: string[];
                warnings: string[];
              }>(SYSTEM_PROMPT, userPrompt, CRIMINAL_APPEAL_JSON_STRUCTURE, 0.3, "sarvam-105b");
            } catch (sarvamErr) {
              console.error("[ai-criminal] Sarvam failed for generateCriminalAppeal, falling back to Groq:", sarvamErr?.message);
              try {
                data = await callGroqStructured<{
                  ...
                  keyPoints: string[];
                  warnings: string[];
                }>(SYSTEM_PROMPT, userPrompt, CRIMINAL_APPEAL_JSON_STRUCTURE, 0.3);
              } catch (groqErr) {
                console.error("[ai-criminal] Groq failed for generateCriminalAppeal, falling back to Gemini:", groqErr?.message);
                const geminiPrompt = `${SYSTEM_PROMPT}\n\nCRITICAL: Respond ONLY with valid JSON:\n${CRIMINAL_APPEAL_JSON_STRUCTURE}`;
                const geminiResponse = await callGeminiText(geminiPrompt, userPrompt, 0.3);

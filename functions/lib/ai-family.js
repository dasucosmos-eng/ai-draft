...
function buildDivorcePrompt(input) {
...
`;
    if (marriageDetails.children && marriageDetails.children.length > 0) {
        prompt += `- **Children:**\n`;
        marriageDetails.children.forEach((child, i) => {
            prompt += `  ${i + 1}. ${child.name}, Age: ${child.age}, Gender: ${child.gender}`;
            if (child.custodyPreference)
                prompt += `, Custody Preference: ${child.custodyPreference}`;
            prompt += `\n`;
        });
    }
    else {
...
`;
    if (domesticViolenceDetails.incidents && domesticViolenceDetails.incidents.length > 0) {
        domesticViolenceDetails.incidents.forEach((inc, i) => {
    ...
    if (domesticViolenceDetails.children && domesticViolenceDetails.children.length > 0) {
        prompt += `\n## Children
${domesticViolenceDetails.children.map((c, i) => `${i + 1}. ${c.name}, Age: ${c.age}`).join("\n")}
`;
    }
    if (domesticViolenceDetails.reliefSought && domesticViolenceDetails.reliefSought.length > 0) {
        prompt += `\n## Specific Reliefs Sought
${domesticViolenceDetails.reliefSought.map((r, i) => `${i + 1}. ${r}`).join("\n")}
`;
    }
    prompt += `
...
function buildMVOPPrompt(input) {
...
## Legal Heirs
${legalHeirs.map((heir, i) => `${i + 1}. **${heir.name}**, Relation: ${heir.relation}, Age: ${heir.age || "Not specified"}, Address: ${heir.address || "Not specified"}, Share: ${heir.share || "To be determined by applicable personal law"}`).join("\n")}
...
function buildGuardianPrompt(input) {
...
## Parent Details
${parentDetails.map((p, i) => `${i + 1}. **${p.name}**, Relation: ${p.relation}, Status: ${p.status}${p.address ? `, Address: ${p.address}` : ""}`).join("\n")}

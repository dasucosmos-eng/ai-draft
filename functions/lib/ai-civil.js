...
`;
    if (matterFacts.properties && matterFacts.properties.length > 0) {
        prompt += `\n## Properties\n`;
        matterFacts.properties.forEach((prop, i) => {
    ...
    if (matterFacts.contracts && matterFacts.contracts.length > 0) {
        ...
        matterFacts.contracts.forEach((c, i) => {
    ...
    if (matterFacts.events && matterFacts.events.length > 0) {
        matterFacts.events.forEach((e, i) => {
    ...
    if (matterFacts.payments && matterFacts.payments.length > 0) {
        prompt += `\n## Payments\n`;
        matterFacts.payments.forEach((p, i) => {
            prompt += `${i + 1}. **${p.date || "Date unspecified"}:** ${p.amount} — ${p.purpose || "Not specified"}\n`;
        });
    }
    prompt += `\n## Reliefs Sought
${(matterFacts.reliefs || []).map((r, i) => `${String.fromCharCode(97 + i)}) ${r}`).join("\n") || "Not specified"}
`;
    if (valuation) {
        prompt += `\n## Valuation & Court Fee
...
// ─── Task: generateWS ──────────────────────────────────────────────────────────
function buildWSPrompt(plaintText, defendantName, defendantCounsel, additionalPleas, setOffCounterClaim) {
...
## Chronology of Events
${(matterFacts.events || []).map((e, i) => `${i + 1}. **${e.date || "Date unspecified"}:** ${e.description}`).join("\n") || "No events provided."}
`;
    if (urgencyFacts) {
...
function buildWrittenArgumentsPrompt(issues, evidence, pleadings, caseType, courtFormat) {
...
// ─── Task: extractFacts ────────────────────────────────────────────────────────
function buildExtractFactsPrompt(text, context) {
            ...
            switch (task) {
                ...
                case "generateWS": {
                    ...
                    try {
                        ...
                        try {
                            data = await (0, groq_client_1.callGroqStructured)(SYSTEM_PROMPT, userPrompt, WS_JSON_STRUCTURE, 0.3);
                        }
                        catch (groqErr) {
                ...
                case "generateInjunctionIA": {
                    ...
                    let data;
                    try {
                        data = await (0, sarvam_client_1.callSarvamStructured)(SYSTEM_PROMPT, userPrompt, INJUNCTION_IA_JSON_STRUCTURE, 0.3, "sarvam-105b");
                    }
                    catch (sarvamErr) {
                        console.error("[ai-civil] Sarvam failed for generateInjunctionIA, falling back to Groq:", sarvamErr?.message);
                        try {
                            data = await (0, groq_client_1.callGroqStructured)(SYSTEM_PROMPT, userPrompt, INJUNCTION_IA_JSON_STRUCTURE, 0.3);
                        }
                        catch (groqErr) {
                ...
                case "generateWrittenArguments": {
                    ...
                    try {
                        ...
                        try {
                            data = await (0, groq_client_1.callGroqStructured)(SYSTEM_PROMPT, userPrompt, WRITTEN_ARGUMENTS_JSON_STRUCTURE, 0.3);
                        }
                        catch (groqErr) {
                ...
                case "extractFacts": {
                    ...
                    let data;
                    try {
                        data = await (0, sarvam_client_1.callSarvamStructured)(SYSTEM_PROMPT, userPrompt, EXTRACT_FACTS_JSON_STRUCTURE, 0.3, "sarvam-105b");
                    }
                    catch (sarvamErr) {
                        console.error("[ai-civil] Sarvam failed for extractFacts, falling back to Groq:", sarvamErr?.message);
                        try {
                            data = await (0, groq_client_1.callGroqStructured)(SYSTEM_PROMPT, userPrompt, EXTRACT_FACTS_JSON_STRUCTURE, 0.3);
                        }
                        catch (groqErr) {

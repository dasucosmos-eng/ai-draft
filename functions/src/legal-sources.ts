...
// ============================================================
// 1. ADVOCATEKHOJ — Bare Acts (FREE, reliable, no CAPTCHA)
// ============================================================
 ...
 */
export async function searchBareActs(query: string): Promise<Array<{ title: string; slug: string }>> {
  try {
 ...
 */
export async function getBareAct(slug: string): Promise<BareAct | null> {
  try {
 ...
 */
export async function getBareActSection(slug: string, sectionNumber: string): Promise<string | null> {
  try {
 ...
 */
export async function searchIndiaCode(query: string): Promise<Array<{ title: string; handle: string }>> {
  try {
    // India Code's simple search returns HTML with results
    const response = await fetch(
      `${INDIA_CODE_BASE}/simple-search?query=${encodeURIComponent(query)}`,
      {
        headers: {
          "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36",
          "Accept": "text/html",
    ...
    const results: Array<{ title: string; handle: string }> = [];
    for (const match of matches) {
 ...
 */
export async function getIndiaCodeAct(handle: string): Promise<string | null> {
  try {
    const response = await fetch(
      `${INDIA_CODE_BASE}/handle/123456789/${handle}/`,
      {
        headers: {
          "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36",
          "Accept": "text/html",
 ...
 */
export const STATUTE_INDEX: Record<string, Array<{
  name: string;
  sections: Array<{ number: string; description: string }>;
  relevance: number;
}>> = {
  "bail": [
    {
 ...
 */
export function getRelevantStatutes(query: string): Array<{
  name: string;
  sections: Array<{ number: string; description: string }>;
  source: string;
  relevance: number;
  ...
  for (const [keyword, statutes] of Object.entries(STATUTE_INDEX)) {
  ...
  const allStatutes = Object.values(STATUTE_INDEX).flat();
 ...
 */
export async function searchAllFreeLegalSources(query: string): Promise<{
  statutes: Array<{
    name: string;
    sections: Array<{ number: string; description: string }>;
    source: string;
  }>;
  bareActs: Array<{ title: string; slug: string; source: string }>;
  indiaCodeResults: Array<{ title: string; handle: string }>;
}> {
  ...
  const bareActs = advocateKhojResults.status === "fulfilled"
    ? advocateKhojResults.value.map((r: { title: string; slug: string }) => ({ ...r, source: "advocatekhoj" }))
    : [];
 ...
 */
export async function getSectionText(slug: string, sectionNumber: string): Promise<string | null> {
...
export async function advocateKhojHealthCheck(): Promise<boolean> {
  try {
    const response = await fetch(`${ADVOCATEKHOJ_BASE}/`, {
...
export async function indiaCodeHealthCheck(): Promise<boolean> {
  try {
...
export async function embeddedStatutesHealthCheck(): Promise<boolean> {
  // Always available — it's in-memory
  return Object.keys(STATUTE_INDEX).length > 0;
}

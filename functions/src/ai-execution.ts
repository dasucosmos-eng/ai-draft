...
function buildEPPrompt(
  decreeDetails: Record<string, any>,
...
function buildParsePrompt(decreeText: string): string {
...
function buildLimitationPrompt(
  decreeDate: string,
  decreeType: string,
  customPeriod?: number
): string {
...
export const apiAiExecution = https.onRequest(
  {
    timeoutSeconds: 180,
          ...
          case "generateSchedule": {
            ...
            try {
              data = await callSarvamStructured<{
                schedule: string;
                memoOfCalculation: string;
              }>(SYSTEM_PROMPT, userPrompt, SCHEDULE_JSON_STRUCTURE, 0.3, "sarvam-105b");
            } catch (sarvamErr) {
              ...
              try {
                data = await callGroqStructured<{
                  schedule: string;
                  memoOfCalculation: string;
                }>(SYSTEM_PROMPT, userPrompt, SCHEDULE_JSON_STRUCTURE, 0.3);
              } catch (groqErr) {
          ...
          case "parseDecree": {
            ...
            try {
              data = await callSarvamStructured<{
                ...
                operativePortion: string;
                courtName: string;
              }>(SYSTEM_PROMPT, userPrompt, PARSE_JSON_STRUCTURE, 0.3, "sarvam-105b");
            } catch (sarvamErr) {
              ...
              try {
                data = await callGroqStructured<{
                  ...
                  operativePortion: string;
                  courtName: string;
                }>(SYSTEM_PROMPT, userPrompt, PARSE_JSON_STRUCTURE, 0.3);
              } catch (groqErr) {
          ...
          case "calculateLimitation": {
            ...
            try {
              data = await callSarvamStructured<{
                ...
                isExpired: boolean;
                warnings: string[];
              }>(SYSTEM_PROMPT, userPrompt, LIMITATION_JSON_STRUCTURE, 0.3, "sarvam-105b");
            } catch (sarvamErr) {
              ...
              try {
                data = await callGroqStructured<{
                  ...
                  isExpired: boolean;
                  warnings: string[];
                }>(SYSTEM_PROMPT, userPrompt, LIMITATION_JSON_STRUCTURE, 0.3);
              } catch (groqErr) {

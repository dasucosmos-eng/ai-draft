...
exports.apiAiExtractData = void 0;
// @ts-nocheck
...
/* ─── Main Handler ─── */
exports.apiAiExtractData = v2_1.https.onRequest({
    timeoutSeconds: 120,
    ...
    return corsHandler(req, res, async () => {
        ...
        try {
            ...
            console.log(`[ai-extract-data] Module: ${mod}, Text length: ${text.length}`);
...
Extract and return the structured data now.`;
            ...
            try {
                ...
                console.error("[ai-extract-data] Sarvam failed, falling back to Groq:", sarvamErr instanceof Error ? sarvamErr.message : sarvamErr);
                try {
                    data = await (0, groq_client_1.callGroqStructured)(systemPrompt, userPrompt, jsonStructure, 0.2);
                    usedProvider = "groq";
                }
                catch (groqErr) {
                    console.error("[ai-extract-data] Groq failed, falling back to Gemini:", groqErr instanceof Error ? groqErr.message : groqErr);
                    // Provider 3: Gemini (reliable fallback)
                    const geminiPrompt = `${systemPrompt}\n\nCRITICAL: Respond ONLY with valid JSON matching this structure:\n${jsonStructure}`;
                    const geminiResponse = await (0, gemini_client_1.callGeminiText)(geminiPrompt, userPrompt, 0.2);
            ...
            (0, groq_client_1.logUsage)("ai-extract-data", undefined, 4000);

...
import { detectLanguage as sarvamDetectLanguage, sarvamChat } from "./sarvam";
import { searchCaseLaw } from "./khanoon";
import { extractTextFromBuffer, performFullOcr } from "./ocr";
...
interface RoutedResponseWithOcr extends RoutedResponse {
  ocrText?: string;
}

interface RoutedResponseWithKhanoon extends RoutedResponse {
  khanoonResults?: Array<{
    id: string;
    title: string;
    court: string;
    year: string;
 ...
 */
function containsIndianLanguage(text: string): boolean {
 ...
 */
function logModelSelection(task: string, model: string, reason: string): void {
 ...
 */
export async function routeDraft(messages: ChatMessage[]): Promise<RoutedResponse> {
  const inputTokens = estimateTokens(
    messages.map((m) => m.content).join(" ")
  );
  ...
  try {
    ...
    const response = await groqChat(messages, { temperature: 0.5 });
    return {
      response,
      model: "groq",
      cost: estimateCostDetailed("groq", "groq_output", inputTokens, estimateTokens(response)),
...
export async function routeDocumentAnalysis(
  ...
  if (isIndian) {
    try {
      const langResult = await sarvamDetectLanguage(extractedContent);
...
Respond ONLY with valid JSON, no markdown or explanation.`,
  ...
  try {
    const response = await groqChat(messages, { temperature: 0.3 });
    return {
      response,
      model: "groq",
      cost: estimateCostDetailed("groq", "groq_output", inputTokens, estimateTokens(response)),
...
export async function routeResearch(
  query: string,
  filters?: { court?: string; year?: string; caseType?: string }
): Promise<RoutedResponseWithKhanoon> {
  const inputTokens = estimateTokens(query);

  // Fetch real case data from Indian Khanoon in parallel (₹0.50/search)
  let khanoonResults: RoutedResponseWithKhanoon["khanoonResults"] = undefined;
  let khanoonCost = 0;
...
export async function routeTranslation(
  ...
  if (indianTargetLangs.includes(targetLang.toLowerCase())) {
    ...
    try {
      ...
      const { detectLanguage: sarvamDetect } = await import("./sarvam");
